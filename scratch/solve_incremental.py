...
def solve_subproblem(active_classes_subset):
    teachers, classes, subjects_map, allocations, ts_set = _fetch_master_data()
    
    # Filter classes
    classes = [c for c in classes if c["nama_kelas"] in active_classes_subset]
...
def run_incremental():
    ...
    try:
        classes = db_fetchall(conn, "SELECT nama_kelas FROM classes ORDER BY nama_kelas")
    finally:
        conn.close()
        
    class_names = [c["nama_kelas"] for c in classes]
    print(f"Total classes to test: {len(class_names)}")
    
    active = []
    for idx, cname in enumerate(class_names):
        active.append(cname)
        status = solve_subproblem(active)
        print(f"[{idx+1}/{len(class_names)}] Classes 1..{idx+1} (added {cname}): {status}")
        if status in ("INFEASIBLE", "INFEASIBLE_OLAH_BLOCK", "INFEASIBLE_NO_CANDIDATES"):
            print(f"\n>>> CONFLICT FOUND! <<<")
            print(f"Feasibility broke when adding class: {cname}")
            print(f"Active subset: {active}")
            break
        elif status == "TIMEOUT":
            print(f"\n>>> TIMEOUT FOUND! <<<")
            print(f"Feasibility solve timed out when adding class: {cname}")
            print(f"Active subset: {active}")
            break
