...
def run_test():
    ...
    # 7. Teacher load limits (WITH min_jp and max_jp constraints)
    teacher_load_vars = {}
    ...
    # Add assumptions variables to debug min_jp / max_jp
    assumptions = []
    assump_desc = {}
    ...
    for tid, load in teacher_load_vars.items():
        ...
        if min_jp_val is not None:
            as_min = new_assump(f"Min JP Guru {t_info['nama_guru']} (min {min_jp_val} JP)")
            model.Add(load >= min_jp_val * is_active).OnlyEnforceIf(as_min)
            if min_jp_val > 10:
                model.Add(is_active == 1).OnlyEnforceIf(as_min)
        
        if max_jp_val is not None:
            as_max = new_assump(f"Max JP Guru {t_info['nama_guru']} (max {max_jp_val} JP)")
            model.Add(load <= max_jp_val).OnlyEnforceIf(as_max)

    model.AddAssumptions(assumptions)
    
    print("Solving model with min_jp and max_jp constraints...")
    ...
    if status == cp_model.INFEASIBLE:
        ...
        for c in core:
            print(f" - {assump_desc.get(c, 'Unknown')}")
    elif status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("FEASIBLE schedule found with min/max JP constraints!")
